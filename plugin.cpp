...
#define PARAM_PHOTOMETRIC_STANDARD_DEVIATION	0
#define PARAM_SPACIAL_DECAY		1
#define PARAM_BILTERALTYPE	2
#define PARAM_ITERATIONS    3

#define NUMBER_OF_USER_PARAMS 4
...
public:
	...
	{
		return "BEEPS or Bi-Exponential Edge-Preserving Smoother is a fast O(N) filter closely related to the bilateral filter.  Using a pair of one TAP recursions and seperating the filter into Horizontal and Vertical components speeds the Bilateral type filter considerably to almost real time.  This filter is closely related to the Domain Transform and SARA filter.";
	}
	...
	int GetUIParameters (UIParameters* pParameters)
	{
		...
		AddParameter( PARAM_PHOTOMETRIC_STANDARD_DEVIATION ,"Photometric SD", .085, .01, 0.5, TYPE_SLIDER, 0.0);
		AddParameter( PARAM_SPACIAL_DECAY ,"Spacial Decay", 0.1, 0.01, .250, TYPE_SLIDER, 0.0);
		AddParameter( PARAM_BILTERALTYPE ,"Gaussian|Hyperbolic Secant|Euler Constant", 0, 0, 2, TYPE_ONEOFMANY, 0);
	...
	{
		//List of Parameters
		double photometricStandardDeviation = GetValue(PARAM_PHOTOMETRIC_STANDARD_DEVIATION);// used to grab 
		double spatialDecay = GetValue(PARAM_SPACIAL_DECAY);// used to grab
		int iterations = GetValue(PARAM_ITERATIONS);// used to grab
		int type = (int)GetValue(PARAM_BILTERALTYPE);
			

		float colorDepth = 255.0;

		float spatialContraDecay = 1.0 - spatialDecay;

		float lambda = exp(-spatialDecay);

		float rho = lambda + spatialDecay / (2.0 - (spatialContraDecay + spatialDecay) );
		...
		if (type == 2)
		{
			...
			c = -pow((0.75 * euler) / (photometricStandardDeviation * photometricStandardDeviation), 1.0 / 3.0);
			c *= (photometricStandardDeviation < 0.0) ? (-1.0) : ((0.0 == photometricStandardDeviation) ? (0.0) : (1.0));			
		}

		

		
		float* iterativeArray = new float[nWidth * nHeight * 4];//working in the 0-1 range
		memset( iterativeArray, 0.0, sizeof(float));
		...
		//http://blog.csdn.net/hjimce/article/details/45420965
		//https://github.com/tuoxie007/BEEPS-objc/blob/master/BEEPS/ViewController.m



		for (int iter = 0; iter < iterations; iter++)
		{
			float* horizontal = new float[nWidth * nHeight * 4];//working in the 0-1 range
			memset( horizontal, 0.0, sizeof(float));
			//copy to horizontal
			for (int x = 0; x < nWidth; x++)
			{
				for (int y = 0; y < nHeight; y++)
				{
					int nIdx = x * 4 + y * 4 * nWidth;
					horizontal[nIdx + CHANNEL_R] = iterativeArray[nIdx + CHANNEL_R];
					horizontal[nIdx + CHANNEL_G] = iterativeArray[nIdx + CHANNEL_G];
					horizontal[nIdx + CHANNEL_B] = iterativeArray[nIdx + CHANNEL_B];
				}
			}

			{//recursiveFilterHorizontal
				//forwards Horizontal PROGRESSIVE
				for (int y = 0; y < nHeight; y++)
				{
					for (int x = 1; x < nWidth; x++)
					{
						int nIdx2 = x * 4 + y * 4 * nWidth;
						int nIdx3 = (x - 1) * 4 + y * 4 * nWidth;

						float muRed = horizontal[nIdx2 + CHANNEL_R] - rho * horizontal[nIdx3 + CHANNEL_R];
						float muGreen = horizontal[nIdx2 + CHANNEL_G] - rho * horizontal[nIdx3 + CHANNEL_G];
						float muBlue = horizontal[nIdx2 + CHANNEL_B] - rho * horizontal[nIdx3 + CHANNEL_B];

						float muRed1 = spatialContraDecay * exp(c * muRed * muRed);
						float muGreen1 = spatialContraDecay * exp(c * muGreen * muGreen);
						float muBlue1 = spatialContraDecay * exp(c * muBlue * muBlue);

						horizontal[nIdx2 + CHANNEL_R] = horizontal[nIdx3 + CHANNEL_R] * muRed1 + horizontal[nIdx2 + CHANNEL_R] * ( 1.0  - muRed1) / rho;
						horizontal[nIdx2 + CHANNEL_G] = horizontal[nIdx3 + CHANNEL_G] * muGreen1 + horizontal[nIdx2 + CHANNEL_G] * ( 1.0  - muGreen1) / rho;
						horizontal[nIdx2 + CHANNEL_B] = horizontal[nIdx3 + CHANNEL_B] * muBlue1 + horizontal[nIdx2 + CHANNEL_B] * ( 1.0  - muBlue1) / rho;
					}//end x

					//backwards horizontal  REGRESSIVE
			...
			float* vertical = new float[nWidth * nHeight * 4];//working in the 0-1 range
			memset( vertical, 0.0, sizeof(float));
			...
			for (int x = 0; x < nWidth; x++)
			{
				...
				{
					int nIdx = x * 4 + y * 4 * nWidth;
					vertical[nIdx + CHANNEL_R] = horizontal[nIdx + CHANNEL_R];
					vertical[nIdx + CHANNEL_G] = horizontal[nIdx + CHANNEL_G];
					vertical[nIdx + CHANNEL_B] = horizontal[nIdx + CHANNEL_B];
				}
			}


			{//recursiveFilterVertical
				//forwards Vertical PROGRESSIVE
				for (int x = 0; x < nWidth; x++)
				{
					...
					{
						int nIdx2 = x * 4 + y * 4 * nWidth;
						int nIdx3 = x * 4 + (y - 1) * 4 * nWidth;

						float muRed = vertical[nIdx2 + CHANNEL_R] - rho * vertical[nIdx3 + CHANNEL_R];
						float muGreen = vertical[nIdx2 + CHANNEL_G] - rho * vertical[nIdx3 + CHANNEL_G];
						float muBlue = vertical[nIdx2 + CHANNEL_B] - rho * vertical[nIdx3 + CHANNEL_B];

						float muRed1 = spatialContraDecay * exp(c * muRed * muRed);
						float muGreen1 = spatialContraDecay * exp(c * muGreen * muGreen);
						float muBlue1 = spatialContraDecay * exp(c * muBlue * muBlue);

						vertical[nIdx2 + CHANNEL_R] = vertical[nIdx3 + CHANNEL_R] * muRed1 + vertical[nIdx2 + CHANNEL_R] * ( 1.0  - muRed1) / rho;
						vertical[nIdx2 + CHANNEL_G] = vertical[nIdx3 + CHANNEL_G] * muGreen1 + vertical[nIdx2 + CHANNEL_G] * ( 1.0  - muGreen1) / rho;
						vertical[nIdx2 + CHANNEL_B] = vertical[nIdx3 + CHANNEL_B] * muBlue1 + vertical[nIdx2 + CHANNEL_B] * ( 1.0  - muBlue1) / rho;
					}//end x
			...
			for (int x = 0; x < nWidth; x++)
			{
				...
				{
					int nIdx = x * 4 + y * 4 * nWidth;
					iterativeArray[nIdx + CHANNEL_R] = vertical[nIdx + CHANNEL_R];
					iterativeArray[nIdx + CHANNEL_G] = vertical[nIdx + CHANNEL_G];
					iterativeArray[nIdx + CHANNEL_B] = vertical[nIdx + CHANNEL_B];
				}
			}
...
#pragma region			//output
		for (int x = 0; x < nWidth; x++)
		{
			for (int y = 0; y < nHeight; y++)
			{
				...
				int nR = iterativeArray [nIdx + CHANNEL_R] * colorDepth;
				int nG = iterativeArray [nIdx + CHANNEL_G] * colorDepth;
				int nB = iterativeArray [nIdx + CHANNEL_B] * colorDepth;
...
#pragma endregion
		...
		delete [] iterativeArray;//delete the array
